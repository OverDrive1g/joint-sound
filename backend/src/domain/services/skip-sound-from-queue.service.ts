...
import {
  SkipSoundFromQueueUseCase,
  SkipSoundError,
} from '../ports/in/skip-sound-from-queue/skip-sound-from-queue.use-case';
...
export class SkipSoundFromQueue implements SkipSoundFromQueueUseCase {
  ...
  async execute(
    command: SkipSoundFromQueueCommand,
  ): Promise<Either<SkipSoundError, void>> {
    ...
    const soundIndex = queue.sounds.findIndex(
      sound => sound.name === command.sound.name,
    );
    if (soundIndex === -1) {
