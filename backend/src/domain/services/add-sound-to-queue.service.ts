...
export class AddSoundToQueueService implements AddSoundToQueueUseCase{
  ...
  async execute(command:AddSoundToQueueCommand):Promise<Either<AddSoundToQueueError, void>>{
    ...
    const loadVideoInfoResult = await this._loadVideoInfo.loadInfo(command.soundSource)
    ...
    if(videoInfo.duration < 50 && videoInfo.duration > 600){
      return left(new AddSoundToQueueError('Video must be loger then 50 sec and shorter then 10 min.'))
    }
    if(videoInfo.viewCount < 3000){
      return left(new AddSoundToQueueError("The video must have more than 3k viewing."))
    }
